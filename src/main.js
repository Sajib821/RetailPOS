// src/main.js — UPDATED version with Supabase multi-store sync
// Replace your existing src/main.js with this file
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    backgroundColor: '#0f1117', show: false,
  });
...
function initDB() {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS products (
      ...
      category TEXT, price REAL NOT NULL, cost REAL DEFAULT 0, stock INTEGER DEFAULT 0,
      low_stock_threshold INTEGER DEFAULT 5, barcode TEXT,
      created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now'))
    );
    CREATE TABLE IF NOT EXISTS sales (
      id INTEGER PRIMARY KEY AUTOINCREMENT, total REAL NOT NULL, subtotal REAL NOT NULL,
      tax REAL DEFAULT 0, discount REAL DEFAULT 0, payment_method TEXT DEFAULT 'cash',
      status TEXT DEFAULT 'completed', note TEXT, created_at TEXT DEFAULT (datetime('now'))
    );
    CREATE TABLE IF NOT EXISTS sale_items (
      ...
      quantity INTEGER NOT NULL, price REAL NOT NULL, subtotal REAL NOT NULL,
      FOREIGN KEY (sale_id) REFERENCES sales(id), FOREIGN KEY (product_id) REFERENCES products(id)
    );
    CREATE TABLE IF NOT EXISTS categories (
      id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE NOT NULL, color TEXT DEFAULT '#6366f1'
    );
  ...
  if (count.c === 0) {
    const ins = db.prepare('INSERT INTO products (name,sku,category,price,cost,stock,low_stock_threshold) VALUES (?,?,?,?,?,?,?)');
    [
...
function initSync() {
  ...
  if (s.supabase_url && s.supabase_key) {
    sync.init({ supabaseUrl: s.supabase_url, supabaseKey: s.supabase_key, storeId: s.store_id, storeName: s.store_name });
    // Push current inventory on startup
    const products = db.prepare('SELECT * FROM products').all();
    sync.syncInventory(products);
  }
}
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('products:search', (_, q) => db.prepare('SELECT * FROM products WHERE name LIKE ? OR sku LIKE ?').all(`%${q}%`, `%${q}%`));
  ipcMain.handle('products:create', (_, p) => {
    const r = db.prepare('INSERT INTO products (name,sku,category,price,cost,stock,low_stock_threshold,barcode) VALUES (?,?,?,?,?,?,?,?)').run(p.name,p.sku,p.category,p.price,p.cost,p.stock,p.low_stock_threshold,p.barcode);
    const all = db.prepare('SELECT * FROM products').all();
    sync.syncInventory(all);
    return r;
  });
  ipcMain.handle('products:update', (_, p) => {
    db.prepare('UPDATE products SET name=?,sku=?,category=?,price=?,cost=?,stock=?,low_stock_threshold=?,barcode=?,updated_at=datetime("now") WHERE id=?').run(p.name,p.sku,p.category,p.price,p.cost,p.stock,p.low_stock_threshold,p.barcode,p.id);
    const all = db.prepare('SELECT * FROM products').all();
    sync.syncInventory(all);
  });
  ...
  ipcMain.handle('categories:create', (_, c) => db.prepare('INSERT OR IGNORE INTO categories (name,color) VALUES (?,?)').run(c.name, c.color));

  // ── Sales ─────────────────────────────────────────────────────────────────
  ipcMain.handle('sales:create', (_, { sale, items }) => {
    const insertSale = db.prepare('INSERT INTO sales (total,subtotal,tax,discount,payment_method,note) VALUES (?,?,?,?,?,?)');
    const insertItem = db.prepare('INSERT INTO sale_items (sale_id,product_id,product_name,quantity,price,subtotal) VALUES (?,?,?,?,?,?)');
    const updateStock = db.prepare('UPDATE products SET stock=stock-? WHERE id=?');

    const tx = db.transaction(() => {
      const result = insertSale.run(sale.total,sale.subtotal,sale.tax,sale.discount,sale.payment_method,sale.note);
    ...
    sync.syncSale({ ...fullSale, id: saleId }, items).catch(() => {});
    const allProducts = db.prepare('SELECT * FROM products').all();
    sync.syncInventory(allProducts).catch(() => {});

    return saleId;
  });

  ipcMain.handle('sales:getAll', (_, { limit=50, offset=0 }={}) => db.prepare('SELECT * FROM sales ORDER BY created_at DESC LIMIT ? OFFSET ?').all(limit, offset));
...
ipcMain.handle('reports:summary', (_, { period='today' }={}) => {
  ...
  // ── Settings ──────────────────────────────────────────────────────────────
  ipcMain.handle('settings:getAll', () => Object.fromEntries(db.prepare('SELECT * FROM settings').all().map(r => [r.key, r.value])));
  ...
  // ── Sync status ───────────────────────────────────────────────────────────
  ipcMain.handle('sync:test', async () => sync.testConnection());
