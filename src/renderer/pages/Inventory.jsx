        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 16 }}>
          {[
            ...
            <div key={s.label} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px' }}>
              <div style={{ color: 'var(--text3)', fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 6 }}>
                {s.icon} {s.label}
              </div>
            ...
            {filtered.map(p => {
              ...
              return (
                <tr key={p.id} style={{ background: 'var(--surface)', animation: 'slideIn 0.2s ease' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div
                        style={{
                          ...
                          height: 8,
                          borderRadius: '50%',
                          background: p.stock === 0 ? 'var(--danger)' : lowStock ? 'var(--accent3)' : 'var(--accent2)',
                          flexShrink: 0
                        }}
                      />
                      {p.name}
                  ...
                  <td style={{ padding: '12px', color: 'var(--text3)', fontSize: 11 }} className="mono">
                    {p.sku || '—'}
                  </td>

                  <td style={{ padding: '12px', textAlign: 'right', color: 'var(--text2)' }} className="mono">
                    {fmt(p.cost)}
                  </td>

                  <td style={{ padding: '12px', textAlign: 'right', fontWeight: 700, fontSize: 15 }} className="mono">
                    <span style={{ color: p.stock === 0 ? 'var(--danger)' : lowStock ? 'var(--accent3)' : 'var(--text)' }}>
                      {p.stock}
                    </span>
                  </td>

                  <td style={{ padding: '12px', textAlign: 'right', color: 'var(--text3)', fontSize: 12 }}>
                    {p.low_stock_threshold}
                  </td>

                  <td style={{ padding: '12px', textAlign: 'right', fontWeight: 600 }} className="mono">
                    {fmt(value)}
                  </td>
        ...
        <Modal title={`Adjust Stock: ${adjustModal.name}`} onClose={() => setAdjustModal(null)}>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', fontSize: 12, color: 'var(--text3)', marginBottom: 6 }}>
              QUANTITY ADJUSTMENT (use negative to remove)
            </label>
            <input
              value={adjQty}
          ...
          >
            <span style={{ color: 'var(--text2)' }}>
              Current: <strong>{adjustModal.stock}</strong>
            </span>
            <span style={{ color: 'var(--text2)' }}>
              → New: <strong style={{ color: 'var(--accent)' }}>{Math.max(0, adjustModal.stock + (parseInt(adjQty, 10) || 0))}</strong>
            </span>
          </div>
